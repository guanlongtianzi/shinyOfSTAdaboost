...
if(getRversion() >= "2.15.1") utils::globalVariables(c('ada_model','pred.ada','gg_pr_Curve','gg_Cumulative_curve','gg_roc_curve','gg_lift_curve'))
...
shinyServer(function(input, output) {
  ...
  output$summary <- renderPrint({
    ...
    ada_model <<- SelfTrain(yuqi~.,train_data,learner('AdaBoostM1',list(control=Weka_control(I=input$ntree))),'pred.ada')
...
```       ',file='AdaboostReport.Rmd',append=F)
  ...
  plot_grid <- function(){
    grid.arrange(gg_pr_Curve, gg_Cumulative_curve, gg_roc_curve, gg_lift_curve, ncol=2, nrow=2, widths=c(2,2), heights=c(2,2),main='Self-Traing Based on Adaboost')
  }
